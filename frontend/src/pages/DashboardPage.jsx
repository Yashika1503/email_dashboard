...
import { Mail, Users, Eye, TrendingUp, AlertCircle, RefreshCw } from 'lucide-react'
import { emailApi, analyticsApi } from "../utils/api";
    ...
    return (
        <div className="stat-card animate-fade-up" style={{ animationDelay: `${delay}ms` }}>
            <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: '0.75rem' }}>
                <span style={{ color: 'var(--ink-muted)', fontSize: '0.82rem', fontWeight: 500 }}>{label}</span>
                <div style={{ width: 32, height: 32, borderRadius: 8, background: 'var(--bg)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                    <Icon size={15} color={color} />
                </div>
            </div>
            <div style={{ fontSize: '2rem', fontFamily: 'DM Serif Display, serif', color: 'var(--ink-primary)', lineHeight: 1.1, marginBottom: '0.25rem' }}>
...
function CustomTooltip({ active, payload, label }) {
    ...
    return (
        <div style={{ background: 'white', border: '1px solid var(--border)', borderRadius: 8, padding: '0.625rem 0.875rem', boxShadow: 'var(--shadow-md)', fontSize: '0.82rem' }}>
...
function PeriodSelector({ value, onChange }) {
    return (
        <div style={{ display: 'flex', gap: '0.25rem', background: 'var(--bg)', borderRadius: 8, padding: '0.2rem' }}>
            {[{ v: 'daily', l: '30d' }, { v: 'weekly', l: '12w' }, { v: 'monthly', l: '6mo' }].map(({ v, l }) => (
...
export default function DashboardPage() {
    ...
    // Use ref so checkSync always sees the latest syncStatus without stale closure
    ...
    async function checkSync() {
        try {
            ...
            // Reload analytics when sync just finished
            if (status.status === 'idle' && prev?.status === 'running') {
                loadAll()
            }
        } catch (err) {
    ...
    const formatVolumeLabel = (label) => {
        if (!label) return ''
        try {
            if (volumePeriod === 'daily') return format(parseISO(label), 'MMM d')
            if (volumePeriod === 'monthly') return format(parseISO(label + '-01'), 'MMM yy')
        } catch { /* ignore */ }
        return label
    }

    const volumeDataKey = volumePeriod === 'daily' ? 'day' : volumePeriod === 'weekly' ? 'week' : 'month'
    ...
    return (
        <div>
            ...
            <div className="animate-fade-up" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '2rem' }}>
                ...
                {syncStatus && (
                    <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', padding: '0.5rem 0.875rem', background: 'white', border: '1px solid var(--border-light)', borderRadius: 8, fontSize: '0.78rem', color: 'var(--ink-muted)' }}>
                        <RefreshCw size={12} style={{ animation: syncStatus.status === 'running' ? 'spin 1s linear infinite' : 'none' }} />
            ...
            {!loading && overview?.totalEmails === 0 && (
                <div style={{ background: 'white', border: '1px solid var(--border-light)', borderRadius: 12, padding: '3rem', textAlign: 'center', marginBottom: '2rem' }}>
                    <Mail size={40} color="var(--ink-muted)" style={{ margin: '0 auto 1rem' }} />
                    <h3 style={{ marginBottom: '0.5rem', fontFamily: 'DM Serif Display, serif' }}>Syncing your emails...</h3>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '1rem', marginBottom: '2rem' }}>
                <StatCard icon={Mail} label="Total Emails" value={overview?.totalEmails?.toLocaleString()} sub={`${overview?.last7Days || 0} in last 7 days`} delay={0} />
                <StatCard icon={Eye} label="Unread" value={overview?.unreadCount?.toLocaleString()} sub={`${100 - (overview?.readRate || 0)}% unread rate`} color="var(--rose)" delay={50} />
                <StatCard icon={TrendingUp} label="Read Rate" value={overview?.readRate != null ? `${overview.readRate}%` : null} sub="of all received emails" color="var(--teal)" delay={100} />
                <StatCard
                    icon={Users} label="7-Day Change"
                    value={overview?.volumeChange != null ? `${overview.volumeChange > 0 ? '+' : ''}${overview.volumeChange}%` : null}
                    sub="vs previous 7 days" color="var(--accent)" delay={150}
                />
            </div>
                    ...
                    {volume?.data ? (
                        <ResponsiveContainer width="100%" height={220}>
                            <AreaChart data={volume.data}>
                                ...
                                <XAxis dataKey={volumeDataKey} tickFormatter={formatVolumeLabel} tick={{ fill: 'var(--ink-muted)', fontSize: 11 }} axisLine={false} tickLine={false} interval="preserveStartEnd" />
                                <YAxis tick={{ fill: 'var(--ink-muted)', fontSize: 11 }} axisLine={false} tickLine={false} />
                                <Tooltip content={<CustomTooltip />} />
                                <Area type="monotone" dataKey="count" stroke="#27231e" strokeWidth={2} fill="url(#volumeGrad)" dot={false} activeDot={{ r: 4, fill: '#27231e' }} />
                            </AreaChart>
                        </ResponsiveContainer>
                    ) : (
                    ...
                    {overview ? (
                        <>
                            ...
                            <div style={{ display: 'flex', justifyContent: 'center', gap: '1.5rem', marginTop: '0.5rem' }}>
                                {[{ label: 'Read', color: '#27231e', value: overview.readCount }, { label: 'Unread', color: '#f59e0b', value: overview.unreadCount }].map(({ label, color, value }) => (
                    ...
                    {senders?.senders ? (
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.625rem' }}>
                            {senders.senders.slice(0, 8).map((sender, i) => {
                                ...
                                return (
                                    <div key={sender.sender_email || i}>
                                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '0.3rem' }}>
                                            <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', flex: 1, minWidth: 0 }}>
                                                <div style={{ width: 22, height: 22, borderRadius: '50%', background: CHART_COLORS[i % CHART_COLORS.length], display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '0.65rem', color: 'white', fontWeight: 700, flexShrink: 0 }}>
                                                    {displayName?.[0]?.toUpperCase() || '?'}
                                                </div>
                                                <span style={{ fontSize: '0.82rem', color: 'var(--ink-secondary)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                                                    {displayName}
                                                </span>
                                            </div>
                                            <span style={{ fontSize: '0.78rem', fontWeight: 600, color: 'var(--ink-primary)', flexShrink: 0, marginLeft: '0.5rem' }}>
                                                {sender.count}
                                            </span>
                                        </div>
                ...
                <div className="card animate-fade-up" style={{ animationDelay: '350ms' }}>
                    ...
                    {labels?.labels ? (
                        <ResponsiveContainer width="100%" height={260}>
                            <BarChart data={labels.labels.slice(0, 8)} layout="vertical" margin={{ left: 8, right: 16 }}>
                                <CartesianGrid strokeDasharray="3 3" stroke="var(--border-light)" horizontal={false} />
                                <XAxis type="number" tick={{ fill: 'var(--ink-muted)', fontSize: 10 }} axisLine={false} tickLine={false} />
                                <YAxis type="category" dataKey="label" width={80} tick={{ fill: 'var(--ink-secondary)', fontSize: 11 }} axisLine={false} tickLine={false} />
                                <Tooltip formatter={(v) => [v.toLocaleString(), 'Emails']} />
                                <Bar dataKey="count" radius={[0, 4, 4, 0]}>
                                    {labels.labels.slice(0, 8).map((entry, i) => (
                                        <Cell key={entry.label} fill={LABEL_COLORS[entry.label] || CHART_COLORS[i % CHART_COLORS.length]} />
                                    ))}
                                </Bar>
                            </BarChart>
                        </ResponsiveContainer>
                ...
                {hourly?.data ? (
                    <ResponsiveContainer width="100%" height={160}>
                        <BarChart data={hourly.data} margin={{ left: 0 }}>
                            ...
                            <XAxis dataKey="hour" tickFormatter={h => h % 6 === 0 ? `${h}:00` : ''} tick={{ fill: 'var(--ink-muted)', fontSize: 10 }} axisLine={false} tickLine={false} />
                            <YAxis tick={{ fill: 'var(--ink-muted)', fontSize: 10 }} axisLine={false} tickLine={false} />
                            <Tooltip formatter={(v) => [v.toLocaleString(), 'Emails']} labelFormatter={(h) => `${h}:00 – ${h + 1}:00`} />
